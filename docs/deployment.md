...
This document describes how to deploy the Unhappy backend (`packages/unhappy-server`) and the infrastructure it expects.
...
- `yarn workspace unhappy-server db` (Postgres in Docker)
- `yarn workspace unhappy-server redis`
- `yarn workspace unhappy-server s3` + `s3:init`

Use `.env`/`.env.dev` to load local settings when running `yarn workspace unhappy-server dev`.

## Implementation references
- Entrypoint: `packages/unhappy-server/sources/main.ts`
- Dockerfile: `Dockerfile.server`
- Kubernetes manifests: `packages/unhappy-server/deploy`
- Env usage: `packages/unhappy-server/sources` (`rg -n "process.env"`)
