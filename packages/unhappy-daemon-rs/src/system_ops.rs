...
fn classify_process(
    ...
    let is_unhappy = is_configured_daemon
        ...
        || command.contains("dist/index.mjs")
        || command.contains("unhappy.mjs")
        || command.contains("unhappy-cli")
        || (command.contains("tsx") && command.contains("src/index.ts") && command.contains("unhappy-cli"));
