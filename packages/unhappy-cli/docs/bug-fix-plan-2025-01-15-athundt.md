# Minimal Fix Plan for Unhappy-CLI Bugs with TDD
...
**Problem**: When running `./bin/unhappy.mjs --continue`, Claude CLI returns error:
...
if (!startFrom && opts.claudeArgs?.includes('--continue')) {
  ...
  if (lastSession) {
    ...
    logger.debug(
      `[ClaudeLocal] Converting --continue to --resume ${lastSession}`,
    );
  } else {
    logger.debug(
      '[ClaudeLocal] No sessions found for --continue, creating new session',
    );
  }
...
## Bug 2: Unhappy Server Unavailability Crash

**Problem**: Unhappy-CLI crashes when Unhappy API server is unreachable

**Server Details**:

- Default server: `https://api.unhappy.im`
- Environment variable: `UNHAPPY_SERVER_URL` (overrides default)
- Local development: `http://localhost:3005`
- The server handles session management and real-time communication for Unhappy CLI
...
try {
  ...
  console.log(
    '⚠️  Cannot connect to Unhappy server - continuing in local mode',
  );
...
```typescript
    ...
    if (error.code === 'ECONNREFUSED' || error.code === 'ENOTFOUND') {
        console.log('⚠️  Unhappy server unreachable - working in offline mode');
...
describe('Api server error handling', () => {
  it('should return null when Unhappy server is unreachable', async () => {
    const consoleSpy = vi.spyOn(console, 'log').mockImplementation(() => {});
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      '⚠️  Unhappy server unreachable - working in offline mode',
    );
...
5. **Refactor Phase**:
   - Add session ID extraction for --continue (future enhancement):
     ...
     - Update Unhappy's session metadata with Claude's ID
...
6. **Manual Verification**:
   ...
   ```bash
   # Test Bug 1 fix:
   ./bin/unhappy.mjs --continue  # Should work without error
   # Verify mobile/daemon still work with session ID

   # Test Bug 2 fix:
   UNHAPPY_SERVER_URL=http://invalid:9999 ./bin/unhappy.mjs  # Should show warning, not crash
...
- Test: `./bin/unhappy.mjs --continue` exits with code 0
- No "session-id cannot be used" error

**Bug 2 Fixed**:

- Test: `UNHAPPY_SERVER_URL=http://invalid:9999 ./bin/unhappy.mjs` shows warning message
- Process continues in local mode instead of crashing
- Clear user feedback: "⚠️ Unhappy server unreachable - working in offline mode"
