 ...
 * through Unhappy CLI. It manages the agent lifecycle, session state, and
 * communication with the Unhappy server and mobile app.
 */
...
export async function runGemini(opts: {
  ...
  try {
    ...
    if (vendorToken?.oauth?.access_token) {
      ...
      logger.debug('[Gemini] Using OAuth token from Unhappy cloud');
      ...
      if (message.meta.model === null) {
        ...
        if (previousModel !== messageModel) {
          ...
          messageBuffer.addMessage(
            `Model changed to: ${messageModel}`,
            'system',
          );
          logger.debug(
            `[Gemini] Model changed from ${previousModel} to ${messageModel}`,
          );
        }
      }
  ...
  const hasExplicitSidechainMarker = (payload: unknown): boolean => {
    if (!payload || typeof payload !== 'object') {
      return false;
    }
    const record = payload as Record<string, unknown>;

    const truthyKeys = ['isSidechain', 'sidechain', 'is_sidechain'];
  ...
  const sendGeminiAgentMessage = (payload: ACPMessageData): void => {
    if (hasExplicitSidechainMarker(payload)) {
  ...
  const updateDisplayedModel = (
    ...
    if (model === undefined) {
      logger.debug(
        `[gemini] updateDisplayedModel called with undefined, skipping update`,
      );
      return;
    }
  ...
  const reasoningProcessor = new GeminiReasoningProcessor((message) => {
    ...
    sendGeminiAgentMessage(message);
  });
    ...
    backend.onMessage((msg: AgentMessage) => {
      switch (msg.type) {
        ...
        case 'status':
          ...
          if (msg.status === 'error') {
            logger.debug(
              `[gemini] ⚠️ Error status received: ${statusDetail || 'Unknown error'}`,
            );
        ...
        case 'tool-call':
          ...
          const isInvestigationTool =
            ...
            (typeof msg.toolName === 'string' &&
              msg.toolName.includes('investigator'));
        ...
        case 'tool-result':
          ...
          const truncatedResult =
            resultText +
            (typeof msg.result === 'string' && msg.result.length > 200
              ? '...'
              : '');
          ...
          if (!isError) {
            diffProcessor.processToolResult(
              msg.toolName,
              msg.result,
              msg.callId,
            );
          ...
          if (isError) {
            ...
            logger.debug(
              `[gemini] ❌ Tool call error: ${errorMsg.substring(0, 300)}`,
            );
        ...
        case 'patch-apply-begin':
          ...
          const filesMsg =
            changeCount === 1 ? '1 file' : `${changeCount} files`;
          messageBuffer.addMessage(`Modifying ${filesMsg}...`, 'tool');
          logger.debug(
            `[gemini] Patch apply begin: ${patchCallId}, files: ${changeCount}`,
          );
        ...
        case 'patch-apply-end':
          ...
          logger.debug(
            `[gemini] Patch apply end: ${patchEndCallId}, success: ${success}`,
          );
        ...
        case 'event':
          ...
          if (msg.name === 'thinking') {
            const thinkingPayload = msg.payload as
              | { text?: string }
              | undefined;
            const thinkingText =
              thinkingPayload &&
              typeof thinkingPayload === 'object' &&
              'text' in thinkingPayload
            ...
            if (thinkingText) {
              ...
              if (!thinkingText.startsWith('**')) {
                ...
                messageBuffer.updateLastMessage(
    ...
    while (!shouldExit) {
      ...
      if (
        ...
        if (conversationHistory.hasHistory()) {
          ...
          messageBuffer.addMessage(
            'Starting new Gemini session (mode changed)...',
            'status',
          );
        }
        ...
        logger.debug(
          '[gemini] Starting new ACP session with model:',
          actualModel,
        );
        ...
        logger.debug(
          `[gemini] Calling updateDisplayedModel with: ${actualModel}`,
        );
      ...
      try {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
          try {
            ...
            if (isQuotaError) {
              ...
              const resetTimeMatch = errorDetails.match(
        ...
        if (isAbortError) {
          ...
          if (typeof error === 'object' && error !== null) {
            ...
            const errorCode =
              errObj.code || errObj.status || errObj.response?.status;
            ...
            // Check for quota/capacity exceeded error
              ...
              // Extract reset time from error message like "Your quota will reset after 3h20m35s."
              const resetTimeMatch = (
                errorDetails +
                errorMessage +
                errorString
              ).match(/reset after (\d+h)?(\d+m)?(\d+s)?/i);
              let resetTimeMsg = '';
              if (resetTimeMatch) {
                const parts = resetTimeMatch.slice(1).filter(Boolean).join('');
                resetTimeMsg = ` Quota resets in ${parts}.`;
        ...
        if (accumulatedResponse.trim()) {
          const { text: messageText, options } =
            parseOptionsFromText(accumulatedResponse);
          ...
          if (options.length > 0) {
            ...
            logger.debug(
              `[gemini] Found ${options.length} options in response:`,
              options,
            );
          ...
          sendGeminiAgentMessage(messagePayload);
          accumulatedResponse = '';
          isResponseInProgress = false;
        }
