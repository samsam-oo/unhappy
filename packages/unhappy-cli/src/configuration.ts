...
class Configuration {
  ...
  constructor() {
    ...
    this.serverUrl = process.env.UNHAPPY_SERVER_URL || 'https://api.cluster-fluster.com'
    this.webappUrl = process.env.UNHAPPY_WEBAPP_URL || 'https://app.happy.engineering'
    ...
    if (process.env.UNHAPPY_HOME_DIR) {
      ...
      const expandedPath = process.env.UNHAPPY_HOME_DIR.replace(/^~/, homedir())
    ...
    this.isExperimentalEnabled = ['true', '1', 'yes'].includes(process.env.UNHAPPY_EXPERIMENTAL?.toLowerCase() || '');
    this.disableCaffeinate = ['true', '1', 'yes'].includes(process.env.UNHAPPY_DISABLE_CAFFEINATE?.toLowerCase() || '');
    ...
    if (variant === 'dev' && !this.happyHomeDir.includes('dev')) {
      console.warn('⚠️  WARNING: UNHAPPY_VARIANT=dev but UNHAPPY_HOME_DIR does not contain "dev"')
