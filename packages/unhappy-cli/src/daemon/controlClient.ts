...
async function daemonPost(path: string, body?: any): Promise<{ error?: string } | any> {
  ...
  try {
    const timeout = process.env.UNHAPPY_DAEMON_HTTP_TIMEOUT ? parseInt(process.env.UNHAPPY_DAEMON_HTTP_TIMEOUT) : 10_000;
...
export async function isDaemonRunningCurrentlyInstalledHappyVersion(): Promise<boolean> {
  ...
  try {
    ...
    // we will get a new path or not when unhappy-coder is upgraded globally.
    ...
    /*
    const { spawnUnhappyCLI } = await import('@/utils/spawnUnhappyCLI');
    const happyProcess = spawnUnhappyCLI(['--version'], { stdio: 'pipe' });
