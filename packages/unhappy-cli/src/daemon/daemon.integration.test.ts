 ...
 * - UNHAPPY_HOME_DIR=~/.unhappy-dev-test (DIFFERENT from dev's ~/.unhappy-dev!)
 * - UNHAPPY_SERVER_URL=http://localhost:3005 (local dev server)
 */
...
describe.skipIf(!await isServerHealthy())('Daemon Integration Tests', { timeout: 20_000 }, () => {
   ...
   * 6. Daemon spawns new daemon via spawnUnhappyCLI(['daemon', 'start'])
   ...
   * 
   * This simulates what happens during `npm upgrade unhappy-coder`:
   * - Running daemon has OLD version loaded in memory (configuration.currentCliVersion)
   * - npm replaces node_modules/unhappy-coder/ with NEW version files
  ...
  it('[takes 1 minute to run] should detect version mismatch and kill old daemon', { timeout: 100_000 }, async () => {
    ...
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, parseInt(process.env.UNHAPPY_DAEMON_HEARTBEAT_INTERVAL || '30000') + 10_000));
  ...
  // TODO: Test npm uninstall scenario - daemon should gracefully handle when unhappy-coder is uninstalled
