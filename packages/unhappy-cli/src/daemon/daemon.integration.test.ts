...
describe.skipIf(!await isServerHealthy())('Daemon Integration Tests', { timeout: 20_000 }, () => {
   ...
   * 
   * This simulates what happens during `npm upgrade unhappy-cli`:
   * - Running daemon has OLD version loaded in memory (configuration.currentCliVersion)
   * - npm replaces node_modules/unhappy-cli/ with NEW version files
  ...
  // TODO: Test npm uninstall scenario - daemon should gracefully handle when unhappy-cli is uninstalled
