 ...
 */
export async function findAllUnhappyProcesses(): Promise<Array<{ pid: number, command: string, type: string }>> {
  try {
    ...
    for (const proc of processes) {
      ...
      const isUnhappy = name.includes('unhappy') ||
                        (name === 'node' && (cmd.includes('unhappy-cli') || cmd.includes('dist/index.mjs'))) ||
                        cmd.includes('unhappy.mjs') ||
                        cmd.includes('unhappy-coder') ||
                        (cmd.includes('tsx') && cmd.includes('src/index.ts') && cmd.includes('unhappy-cli'));
...
/**
 * Find all runaway Unhappy CLI processes that should be killed
 */
export async function findRunawayUnhappyProcesses(): Promise<Array<{ pid: number, command: string }>> {
...
export async function killRunawayHappyProcesses(): Promise<{ killed: number, errors: Array<{ pid: number, error: string }> }> {
  const runawayProcesses = await findRunawayUnhappyProcesses();
