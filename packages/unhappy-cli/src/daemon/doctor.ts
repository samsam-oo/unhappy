 ...
 */
export async function findAllUnhappyProcesses(): Promise<
 ...
 */
export async function findRunawayUnhappyProcesses(): Promise<
...
/**
 * Kill all runaway Unhappy CLI processes
 */
  ...
  for (const { pid, command } of runawayProcesses) {
    try {
      ...
      if (process.platform === 'win32') {
        ...
        const result = spawn.sync('taskkill', ['/F', '/PID', pid.toString()], {
          stdio: 'pipe',
        });
