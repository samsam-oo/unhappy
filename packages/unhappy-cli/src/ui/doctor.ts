...
import { findRunawayUnhappyProcesses, findAllUnhappyProcesses } from '@/daemon/doctor'
...
export async function runDoctorCommand(filter?: 'all' | 'daemon'): Promise<void> {
    ...
    if (filter === 'all') {
        ...
        console.log(`UNHAPPY_HOME_DIR: ${env.UNHAPPY_HOME_DIR ? chalk.green(env.UNHAPPY_HOME_DIR) : chalk.gray('not set')}`);
        console.log(`UNHAPPY_SERVER_URL: ${env.UNHAPPY_SERVER_URL ? chalk.green(env.UNHAPPY_SERVER_URL) : chalk.gray('not set')}`);
    ...
    if (filter === 'all') {
        ...
        console.log(chalk.bold('\n🐛 Support & Bug Reports'));
        console.log(`Report issues: ${chalk.blue('https://github.com/samsam-oo/unhappy-cli/issues')}`);
