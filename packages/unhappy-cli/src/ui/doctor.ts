...
export function getEnvironmentInfo(): Record<string, any> {
  return {
    PWD: process.env.PWD,
    UNHAPPY_HOME_DIR: process.env.UNHAPPY_HOME_DIR,
    UNHAPPY_SERVER_URL: process.env.UNHAPPY_SERVER_URL,
    UNHAPPY_PROJECT_ROOT: process.env.UNHAPPY_PROJECT_ROOT,
...
function getLogFiles(
  logDir: string,
): { file: string; path: string; modified: Date }[] {
...
export async function runDoctorCommand(
  ...
  if (filter === 'all') {
    ...
    console.log(`Unhappy CLI Version: ${chalk.green(packageJson.version)}`);
    ...
    const wrapperPath = join(projectRoot, 'bin', 'unhappy.mjs');
    ...
    console.log(
      `UNHAPPY_HOME_DIR: ${env.UNHAPPY_HOME_DIR ? chalk.green(env.UNHAPPY_HOME_DIR) : chalk.gray('not set')}`,
    );
    console.log(
      `UNHAPPY_SERVER_URL: ${env.UNHAPPY_SERVER_URL ? chalk.green(env.UNHAPPY_SERVER_URL) : chalk.gray('not set')}`,
    );
    console.log(
      `DANGEROUSLY_LOG_TO_SERVER: ${env.DANGEROUSLY_LOG_TO_SERVER_FOR_AI_AUTO_DEBUGGING ? chalk.yellow('ENABLED') : chalk.gray('not set')}`,
    );
    console.log(
      `DEBUG: ${env.DEBUG ? chalk.green(env.DEBUG) : chalk.gray('not set')}`,
    );
    console.log(
      `NODE_ENV: ${env.NODE_ENV ? chalk.green(env.NODE_ENV) : chalk.gray('not set')}`,
    );
  ...
  try {
    ...
    if (isRunning && state) {
      ...
      console.log(
        chalk.yellow('⚠️  Daemon state exists but process not running (stale)'),
      );
    } else {
  ...
  if (filter === 'all') {
    ...
    if (allLogs.length > 0) {
      ...
      const regularLogs = allLogs.filter(
        ({ file }) => !file.includes('daemon'),
      );
      ...
      if (regularLogs.length > 0) {
        ...
        if (regularLogs.length > 10) {
          console.log(
            chalk.gray(`  ... and ${regularLogs.length - 10} more log files`),
          );
        }
      }
      ...
      if (daemonLogs.length > 0) {
        ...
        if (daemonLogs.length > 5) {
          console.log(
            chalk.gray(
              `  ... and ${daemonLogs.length - 5} more daemon log files`,
            ),
          );
        }
      } else {
