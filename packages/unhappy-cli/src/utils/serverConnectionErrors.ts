...
export interface OfflineReconnectionConfig<TSession> {
    /** Server URL to health-check against (e.g., 'https://api.unhappy.im') */
    serverUrl: string;
    ...
    /**
     * Optional: computes retry delay (ms) for a given failure count.
     * Default uses exponential backoff capped around 60s.
     * Useful for deterministic/fast unit tests.
     */
    computeRetryDelayMs?: (failureCount: number) => number;
}
    ...
    const attemptReconnect = async () => {
        ...
        try {
            ...
            if (axios.isAxiosError(e) && e.response?.status === 401) {
                ...
                config.onNotify('❌ Authentication failed. Please re-authenticate with `unhappy auth`.');
            ...
            failureCount++;
            const delay = computeRetryDelayMs(failureCount);
...
export const ERROR_DESCRIPTIONS: Record<string, string> = {
    ...
    // HTTP errors
    '401': 'authentication failed - run `unhappy auth`',
...
class OfflineState {
    ...
    private print(): void {
        const summary = [...this.failures.values()]
            ...
            .join('; ');
        console.log(`⚠️  Unhappy server unreachable, offline mode with auto-reconnect enabled - error details: ${summary}`);
