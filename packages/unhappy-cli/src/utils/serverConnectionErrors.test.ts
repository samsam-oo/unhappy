    ...
    describe('error handling', () => {
        it('should stop retrying on 401 authentication error', async () => {
            ...
            expect(onNotify).toHaveBeenCalledWith(
                '❌ Authentication failed. Please re-authenticate with `unhappy auth`.'
            );
            expect(onReconnected).not.toHaveBeenCalled();
...
describe('printOfflineWarning', () => {
    ...
    it('should print offline warning with unified format', () => {
        ...
        expect(consoleSpy).toHaveBeenCalledWith(
            expect.stringContaining('⚠️  Unhappy server unreachable, offline mode with auto-reconnect enabled')
        );
        expect(consoleSpy).toHaveBeenCalledWith(
