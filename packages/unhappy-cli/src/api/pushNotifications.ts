...
export class PushNotificationClient {
  ...
  constructor(token: string, baseUrl: string = 'https://api.unhappy.im') {
    this.token = token;
    ...
    for (const chunk of chunks) {
      ...
      while (true) {
        try {
          ...
          const errors = ticketChunk.filter(
          ...
          if (elapsed >= timeout) {
            logger.debug(
              '[PUSH] Timeout reached after 5 minutes, giving up on chunk',
            );
            break;
          }
          ...
          if (waitTime > 0) {
            logger.debug(
              `[PUSH] Retrying in ${waitTime}ms (attempt ${attempt})`,
            );
  ...
  sendToAllDevices(
    ...
    logger.debug(
      `[PUSH] sendToAllDevices called with title: "${title}", body: "${body}"`,
    );
