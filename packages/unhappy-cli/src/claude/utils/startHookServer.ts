 ...
 *     │                                         
 *     ├─► generateHookSettingsFile(port) ──► ~/.unhappy/tmp/hooks/session-hook-<pid>.json
 ...
 * 
 * ### Triggered By
 * - `unhappy` (fresh start) - new session created
 * - `unhappy --continue` - continues last session (may fork)
 * - `unhappy --resume` - interactive picker, then resume
 * - `unhappy --resume <id>` - resume specific session
 * - `/compact` command - compacts and forks session
 * - Double-escape fork - user forks conversation in CLI
 * 
 * ### Why Not Use File Watching?
 * File watching has race conditions when multiple Unhappy processes run.
 * With hooks, Claude directly tells THIS specific process about its session,
 * ensuring 1:1 mapping between Unhappy process and Claude session.
 */
