 ...
 * - UNHAPPY_CLAUDE_PATH: Force a specific path to claude executable
 * - UNHAPPY_USE_BUNDLED_CLAUDE=1: Force use of node_modules version (skip global search)
 * - UNHAPPY_USE_GLOBAL_CLAUDE=1: Force use of global version (if available)
 */
export function getDefaultClaudeCodePath(): string {
    ...
    if (process.env.UNHAPPY_CLAUDE_PATH) {
        logger.debug(`[Claude SDK] Using UNHAPPY_CLAUDE_PATH: ${process.env.UNHAPPY_CLAUDE_PATH}`)
        return process.env.UNHAPPY_CLAUDE_PATH
    }

    // Force bundled version if requested
    if (process.env.UNHAPPY_USE_BUNDLED_CLAUDE === '1') {
