...
```bash
git clone https://github.com/samsam-oo/unhappy-cli.git
cd unhappy-cli
yarn install
yarn build
```
...
### Global `unhappy-dev` Command

Create a global `unhappy-dev` command that runs your local development build:

```bash
yarn link:dev      # Create unhappy-dev symlink
yarn unlink:dev    # Remove unhappy-dev symlink
```
...
3. Now `cd` into the directory automatically sets `UNHAPPY_VARIANT=dev`!
...
The system uses the built-in `UNHAPPY_HOME_DIR` environment variable to separate data:

- **Stable scripts** set: `UNHAPPY_HOME_DIR=~/.unhappy`
- **Dev scripts** set: `UNHAPPY_HOME_DIR=~/.unhappy-dev`

Everything else (auth, sessions, logs, daemon) automatically follows the `UNHAPPY_HOME_DIR` setting.
...
2. **Verify CLI receives profile:**
   ...
   # Check daemon logs
   tail -f ~/.unhappy-dev/logs/*.log | grep -i profile
   ```
