...
const STABLE_DIR = path.join(os.homedir(), '.unhappy');
const DEV_DIR = path.join(os.homedir(), '.unhappy-dev');

console.log('🔧 Setting up unhappy-cli development environment...\n');
...
const envrcContent = `# Unhappy CLI environment (for direnv users)
# Automatically sets UNHAPPY_HOME_DIR based on directory
#
# To use: cd to unhappy-cli-dev directory, run: direnv allow
export UNHAPPY_HOME_DIR="$HOME/.unhappy-dev"
export UNHAPPY_VARIANT="dev"
`;
