...
describe('UNHAPPY_CLAUDE_PATH env var', () => {
  ...
  afterEach(() => {
    ...
    delete process.env.UNHAPPY_CLAUDE_PATH;
  });

  it('should use UNHAPPY_CLAUDE_PATH when set', () => {
    process.env.UNHAPPY_CLAUDE_PATH = testClaudePath;
    const result = findGlobalClaudeCliPath();
    expect(result?.source).toBe('UNHAPPY_CLAUDE_PATH');
  ...
  it('should fall back to auto-discovery when env var not set', () => {
    const result = findGlobalClaudeCliPath();
    expect(result?.source).not.toBe('UNHAPPY_CLAUDE_PATH');
  });

  it('should ignore env var if path does not exist', () => {
    process.env.UNHAPPY_CLAUDE_PATH = '/nonexistent/path/claude';
    const result = findGlobalClaudeCliPath();
    expect(result?.source).not.toBe('UNHAPPY_CLAUDE_PATH');
  });
});