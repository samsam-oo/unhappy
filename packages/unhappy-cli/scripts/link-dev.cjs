#!/usr/bin/env node
/**
 * link-dev.cjs - Create symlink for unhappy-dev only
 *
 * This script creates a symlink for the unhappy-dev command pointing to the local
 * development version, while leaving the stable npm version of `unhappy` untouched.
 *
 * Usage: yarn link:dev
 *
 * What it does:
 * 1. Finds the global npm bin directory
 * 2. Creates/updates a symlink: unhappy-dev -> ./bin/unhappy-dev.mjs
 *
...
const binSource = join(projectRoot, 'bin', 'unhappy-dev.mjs');
...
function link() {
    ...
    const binTarget = join(globalBin, 'unhappy-dev');

    console.log('Creating symlink for unhappy-dev...');
    ...
    try {
        ...
        console.log('\n✅ Successfully linked unhappy-dev to local development version');
        console.log('\nNow you can use:');
        console.log('  unhappy      → stable npm version (unchanged)');
        console.log('  unhappy-dev  → local development version');
        console.log('\nTo undo: yarn unlink:dev');
    } catch (e) {
        if (e.code === 'EACCES') {
...
function unlink() {
    ...
    console.log('Removing unhappy-dev symlink...');
    ...
    try {
        ...
        if (e.code === 'ENOENT') {
            console.log("\n✅ unhappy-dev symlink doesn't exist (already removed or never created)");
