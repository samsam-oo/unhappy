/**
 * Chinese (Simplified) translations for the Unhappy app
 * Values can be:
...
export const zhHans: TranslationStructure = {
  ...
  settings: {
    ...
    about: '关于',
    aboutFooter:
      'Unhappy Coder 是一个 Codex 和 Claude Code 移动客户端。它采用端到端加密，您的账户仅存储在本地设备上。与 Anthropic 无关联。',
    whatsNew: '更新日志',
  ...
  newSession: {
    ...
    title: '启动新会话',
    noMachinesFound: '未找到设备。请先在您的计算机上启动 Unhappy 会话。',
  ...
  server: {
    ...
    notValidHappyServer: '不是有效的 Unhappy 服务器',
    changeServer: '更改服务器',
  ...
  sessionInfo: {
    ...
    happySessionIdCopied: 'Unhappy 会话 ID 已复制到剪贴板',
    failedToCopySessionId: '复制 Unhappy 会话 ID 失败',
    happySessionId: 'Unhappy 会话 ID',
    ...
    updateCliInstructions: '请运行 npm install -g unhappy-coder@latest',
    deleteSession: '删除会话',
  ...
  terminal: {
    ...
    terminalRequestDescription:
      '有终端正在请求连接到您的 Unhappy Coder 账户。这将允许终端安全地发送和接收消息。',
  ...
  machine: {
    ...
    offlineHelp:
      '• 确保您的计算机在线\n• 运行 `unhappy daemon status` 进行诊断\n• 您是否在运行最新的 CLI 版本？请使用 `npm install -g unhappy-coder@latest` 升级',
    daemon: '守护进程',
    status: '状态',
    stopDaemon: '停止守护进程',
  ...
  finishSession: {
    title: '完成会话',
    subtitle: '合并、创建 PR 或清理工作树',
    branchName: '分支',
    basePath: '基础仓库',
    uncommittedWarning: '此工作树有未提交的更改',
    commitChanges: '提交更改',
    commitChangesSubtitle: '暂存所有更改并创建一次提交',
    commitMessageTitle: '提交信息',
    commitMessagePrompt: '请输入提交信息：',
    commitConfirm: '提交',
    commitMessageRequired: '提交信息为必填。',
    noChangesToCommit: '没有可提交的更改。',
    mustCommitBeforePush: '有未提交的更改。推送前请先提交或暂存。',
    nothingToPush: '没有可推送的提交。请先提交更改。',
    commitSuccess: '提交已创建',
    commitSuccessMessage: '更改已成功提交。',
    actions: '操作',
    mergeInto: ({ branch }: { branch: string }) => `合并到 ${branch}`,
    mergeSubtitle: ({ branch }: { branch: string }) =>
      `将此分支合并到 ${branch}`,
    mergeConfirmTitle: '合并分支',
    mergeConfirmMessage: ({
      branch,
      target,
    }: {
    ...
    merge: '合并',
    pushAfterMerge: '合并后推送',
    mergeSuccess: '合并完成',
    mergeSuccessMessage: '分支已成功合并。',
    mergeAndPushSuccessMessage: '分支已成功合并并推送。',
    createPR: '创建 Pull Request',
    createPRSubtitle: '推送分支并在 GitHub 上创建 PR',
    prCreated: 'Pull Request 已创建',
    copyUrl: '复制 URL',
    dangerZone: '危险区域',
    deleteWorktree: '删除工作树',
    deleteWorktreeSubtitle: '移除工作树、删除分支并清理',
    deleteConfirmTitle: '删除工作树',
    deleteConfirmMessage: ({ branch }: { branch: string }) =>
      `将移除 '${branch}' 工作树并删除分支。此操作无法撤销。`,
    deleteSuccess: '工作树已删除',
    deleteSuccessMessage: '工作树和分支已被移除。',
    notAWorktree: '此会话不是工作树会话。',
