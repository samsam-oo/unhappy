...
/**
 * Russian translations for the Unhappy app
...
export const ru: TranslationStructure = {
  ...
  settings: {
    ...
    about: 'О программе',
    aboutFooter:
      'Unhappy Coder — мобильное приложение для работы с Codex и Claude Code. Использует сквозное шифрование, все данные аккаунта хранятся только на вашем устройстве. Не связано с Anthropic.',
    whatsNew: 'Что нового',
  ...
  newSession: {
    ...
    noMachinesFound:
      'Машины не найдены. Сначала запустите сессию Unhappy на вашем компьютере.',
  ...
  server: {
    ...
    notValidHappyServer: 'Это не валидный сервер Unhappy',
  ...
  sessionInfo: {
    ...
    happySessionIdCopied: 'ID сессии Unhappy скопирован в буфер обмена',
    failedToCopySessionId: 'Не удалось скопировать ID сессии Unhappy',
    happySessionId: 'ID сессии Unhappy',
    ...
    happyHome: 'Домашний каталог Unhappy',
  ...
  components: {
    emptyMainScreen: {
      ...
      installCli: 'Установите Unhappy CLI',
      runIt: 'Запустите его',
  ...
  terminal: {
    ...
    terminalRequestDescription:
      'Терминал запрашивает подключение к вашему аккаунту Unhappy Coder. Это позволит терминалу безопасно отправлять и получать сообщения.',
  ...
  finishSession: {
    ...
    commitChangesSubtitle: 'Добавить все изменения в stage и создать коммит',
    commitMessageTitle: 'Сообщение коммита',
    commitMessagePrompt: 'Введите сообщение коммита:',
    commitConfirm: 'Коммит',
    commitMessageRequired: 'Сообщение коммита обязательно.',
    noChangesToCommit: 'Нет изменений для коммита.',
    mustCommitBeforePush:
      'Есть незакоммиченные изменения. Сделайте коммит или stash перед push.',
    nothingToPush: 'Нет коммитов для push. Сначала закоммитьте изменения.',
    commitSuccess: 'Коммит создан',
    commitSuccessMessage: 'Изменения успешно закоммичены.',
    actions: 'Действия',
    mergeInto: ({ branch }: { branch: string }) => `Слить в ${branch}`,
    mergeSubtitle: ({ branch }: { branch: string }) =>
      `Слить эту ветку в ${branch}`,
