...
| `friends/index.tsx` | Partial | Inbox toolbar에서 Friends 전용 **독립 화면**으로 `Pending Requests`/`Sent Requests`/`Friends` 섹션 + accept/reject/cancel/remove 액션 제공. web route-level 화면 구조/스타일 parity는 미구현. |
| `friends/search.tsx` | Partial | Inbox/Friends 화면에서 `Find Friends` **독립 화면**으로 user search + add/cancel/accept 액션 제공(`/v1/user/search`, `/v1/friends/add`, `/v1/friends/remove`). web dedicated screen parity는 미구현. |
| `user/[id].tsx` | Partial | Inbox/Friends/Search에서 user detail **독립 화면**으로 진입 가능하며 관계 상태별 액션(add/accept/reject/cancel/remove) 제공(`/v1/user/:id`). web route-level 화면 구성 parity는 미구현. |
...
| `apiFriends` (`/v1/friends`, `/v1/friends/add`, `/v1/friends/remove`) | Partial | friends list + add/remove API 브릿지 및 inbox action 연동 구현. |
| `apiUser` (`/v1/user/:id`, `/v1/user/search`) | Partial | user detail/search API 브릿지 및 inbox profile/search sheet 연동 구현. dedicated friends/user 화면 parity는 미구현. |
