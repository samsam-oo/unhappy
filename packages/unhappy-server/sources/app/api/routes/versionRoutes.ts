    ...
    app.post('/v1/version', {
        ...
        if (platform.toLowerCase() === 'ios') {
            if (semver.satisfies(version, IOS_UP_TO_DATE)) {
                ...
                reply.send({ updateUrl: 'https://apps.apple.com/us/app/unhappy-claude-code-client/id6748571505' });
            }
            return;
        }
        ...
        if (platform.toLowerCase() === 'android') {
            if (semver.satisfies(version, ANDROID_UP_TO_DATE)) {
                ...
                reply.send({ updateUrl: 'https://play.google.com/store/apps/details?id=im.unhappy.app' });
            }
            return;
        }
