...
/**
 * Korean translations for the Happy app
 * Values can be:
...
export const ko: TranslationStructure = {
    ...
    settings: {
        ...
        about: '정보',
        aboutFooter:
            "Happy Coder는 Codex와 Claude Code용 모바일 클라이언트입니다. 종단 간 암호화를 제공하며 계정 정보는 기기에만 저장됩니다. Anthropic과는 관련이 없습니다.",
        whatsNew: '새로운 소식',
        ...
        accountConnected: ({ service }: { service: string }) => `${service} 계정이 연결되었습니다`,
    ...
    errors: {
        ...
        errorWithCode: ({ message, code }: { message: string; code: number | string }) =>
            `${message} (오류 ${code})`,
        disconnectServiceFailed: ({ service }: { service: string }) => `${service} 연결 해제에 실패했습니다`,
    ...
    newSession: {
        ...
        title: '새 세션 시작',
        noMachinesFound: '머신을 찾을 수 없습니다. 먼저 컴퓨터에서 Happy 세션을 시작하세요.',
        ...
        sessionSpawningFailed: '세션 생성에 실패했습니다 (세션 ID가 반환되지 않았습니다).',
        startingSession: '세션 시작 중...',
        startNewSessionInFolder: '여기서 새 세션',
        failedToStart: '세션 시작에 실패했습니다. 대상 머신에서 데몬이 실행 중인지 확인하세요.',
        sessionTimeout: '세션 시작 시간이 초과되었습니다. 머신이 느리거나 데몬이 응답하지 않을 수 있습니다.',
        notConnectedToServer: '서버에 연결되지 않았습니다. 인터넷 연결을 확인하세요.',
        ...
        worktree: {
            ...
            failed: ({ error }: { error: string }) => `워크트리 생성에 실패했습니다: ${error}`,
    ...
    server: {
        ...
        notValidHappyServer: '유효한 Happy Server가 아닙니다',
        changeServer: '서버 변경',
    ...
    sessionInfo: {
        ...
        happySessionIdCopied: 'Happy 세션 ID가 클립보드에 복사되었습니다',
        failedToCopySessionId: 'Happy 세션 ID 복사에 실패했습니다',
        happySessionId: 'Happy 세션 ID',
        claudeCodeSessionId: 'Claude Code 세션 ID',
        claudeCodeSessionIdCopied: 'Claude Code 세션 ID가 클립보드에 복사되었습니다',
        aiProvider: 'AI 제공자',
        ...
        updateCliInstructions: 'npm install -g happy-coder@latest 를 실행하세요',
        deleteSession: '세션 삭제',
    ...
    tools: {
        ...
        multiEdit: {
            editNumber: ({ index, total }: { index: number; total: number }) => `${total}개 중 ${index}번째 편집`,
            replaceAll: '모두 바꾸기',
        },
        names: {
            task: '작업',
        ...
        desc: {
            ...
            todoListCount: ({ count }: { count: number }) => `할 일 목록(count: ${count})`,
            webSearchQuery: ({ query }: { query: string }) => `웹 검색(query: ${query})`,
            grepPattern: ({ pattern }: { pattern: string }) => `grep(pattern: ${pattern})`,
            multiEditEdits: ({ path, count }: { path: string; count: number }) => `${path} (${count}개 편집)`,
            readingFile: ({ file }: { file: string }) => `${file} 읽는 중`,
            writingFile: ({ file }: { file: string }) => `${file} 쓰는 중`,
            modifyingFile: ({ file }: { file: string }) => `${file} 수정 중`,
            modifyingFiles: ({ count }: { count: number }) => `${count}개 파일 수정 중`,
            modifyingMultipleFiles: ({ file, count }: { file: string; count: number }) => `${file} 외 ${count}개`,
    ...
    files: {
        ...
        summary: ({ staged, unstaged }: { staged: number; unstaged: number }) => `${staged}개 스테이징 • ${unstaged}개 미스테이징`,
        ...
        projectRoot: '프로젝트 루트',
        stagedChanges: ({ count }: { count: number }) => `스테이징된 변경 사항 (${count})`,
        unstagedChanges: ({ count }: { count: number }) => `미스테이징 변경 사항 (${count})`,
        // File viewer strings
        loadingFile: ({ fileName }: { fileName: string }) => `${fileName} 로딩 중...`,
        binaryFile: '바이너리 파일',
    ...
    settingsVoice: {
        ...
        languageTitle: '언어',
        languageDescription: '음성 어시스턴트와의 상호작용에 사용할 언어를 선택하세요. 이 설정은 모든 기기에서 동기화됩니다.',
    ...
    settingsAccount: {
        ...
        secretKeyCopied: '비밀 키가 클립보드에 복사되었습니다. 안전한 곳에 보관하세요!',
    ...
    settingsLanguage: {
        ...
        title: '언어',
        description: '앱 인터페이스에 사용할 언어를 선택하세요. 이 설정은 모든 기기에서 동기화됩니다.',
    ...
    terminal: {
        ...
        invalidConnectionLinkDescription: '연결 링크가 없거나 올바르지 않습니다. URL을 확인하고 다시 시도하세요.',
        connectTerminal: '터미널 연결',
        terminalRequestDescription:
            '터미널이 Happy Coder 계정에 연결을 요청하고 있습니다. 이 연결을 통해 터미널이 안전하게 메시지를 송수신할 수 있습니다.',
    ...
    modals: {
        ...
        disconnectService: ({ service }: { service: string }) => `${service} 연결 해제`,
        disconnectServiceConfirm: ({ service }: { service: string }) =>
            `계정에서 ${service} 연결을 해제하시겠습니까?`,
        disconnect: '연결 해제',
        failedToConnectTerminal: '터미널 연결에 실패했습니다',
        cameraPermissionsRequiredToConnectTerminal: '터미널을 연결하려면 카메라 권한이 필요합니다',
        failedToLinkDevice: '기기 연결에 실패했습니다',
        cameraPermissionsRequiredToScanQr: 'QR 코드를 스캔하려면 카메라 권한이 필요합니다',
    },
    ...
    items: {
        ...
        copiedToClipboard: ({ label }: { label: string }) => `${label}이(가) 클립보드에 복사되었습니다`,
    },
    ...
    machine: {
        ...
        offlineHelp:
            '• 컴퓨터가 온라인인지 확인하세요\n• `happy daemon status`를 실행하여 진단하세요\n• 최신 CLI 버전인가요? `npm install -g happy-coder@latest`로 업그레이드하세요',
        daemon: '데몬',
        status: '상태',
        stopDaemon: '데몬 중지',
    ...
    message: {
        switchedToMode: ({ mode }: { mode: string }) => `${mode} 모드로 전환했습니다`,
        unknownEvent: '알 수 없는 이벤트',
        usageLimitUntil: ({ time }: { time: string }) => `${time}까지 사용량 제한이 적용됩니다`,
    ...
    artifacts: {
        ...
        discardChangesDescription: '저장되지 않은 변경 사항이 있습니다. 정말로 버리시겠습니까?',
    ...
    friends: {
        ...
        bothMustHaveGithub: '두 사용자 모두 GitHub가 연결되어 있어야 친구가 될 수 있습니다',
        status: {
            none: '연결되지 않음',
            requested: '요청 보냄',
            pending: '요청 대기',
        ...
        acceptRequest: '요청 수락',
        removeFriend: '친구 삭제',
        removeFriendConfirm: ({ name }: { name: string }) => `${name}을(를) 친구에서 삭제하시겠습니까?`,
        requestSentDescription: ({ name }: { name: string }) => `${name}에게 친구 요청을 보냈습니다`,
        requestFriendship: '친구 요청',
        cancelRequest: '친구 요청 취소',
        cancelRequestConfirm: ({ name }: { name: string }) => `${name}에게 보낸 친구 요청을 취소할까요?`,
        denyRequest: '친구 요청 거절',
        nowFriendsWith: ({ name }: { name: string }) => `${name}과(와) 친구가 되었습니다`,
    },
    ...
    feed: {
        ...
        friendRequestFrom: ({ name }: { name: string }) => `${name}님이 친구 요청을 보냈습니다`,
        friendRequestGeneric: '새 친구 요청',
        friendAccepted: ({ name }: { name: string }) => `${name}님과 친구가 되었습니다`,
    ...
    profiles: {
        ...
        delete: {
            title: '프로필 삭제',
            message: ({ name }: { name: string }) => `"${name}"을(를) 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다.`,
            confirm: '삭제',
            cancel: '취소',
        },
    },
